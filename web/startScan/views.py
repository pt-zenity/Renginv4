...
def start_scan_ui(request, slug, domain_id):
    ...
    if request.method == "POST":
        ...
        engine_id = safe_int_cast(request.POST.get("scan_mode", 0))
...
def start_multiple_scan(request, slug):
    ...
    if request.method == "POST":
        if request.POST.get("scan_mode", 0):
            ...
            engine_id = safe_int_cast(request.POST.get("scan_mode", 0))
            scan_type = request.POST.get("scan_type", "bug_bounty")
            list_of_domains = request.POST.get("list_of_domain_id", "")
...
def schedule_scan(request, host_id, slug):
    ...
    if request.method == "POST":
        scheduled_mode = request.POST.get("scheduled_mode", "periodic")
        engine_type = int(request.POST.get("scan_mode", 0))
        ...
        if scheduled_mode == "periodic":
            frequency_value = int(request.POST.get("frequency", 1))
            frequency_type = request.POST.get("frequency_type", "days")
