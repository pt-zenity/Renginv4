...
# ── Redis cache connection (lazy, with reconnect on failure) ──────────────────
_cache_lock = threading.Lock()
_cache_instance = None


def _get_cache():
    """Return a Redis cache connection, creating/reconnecting as needed."""
    global _cache_instance
    if "CELERY_BROKER" not in os.environ:
        return None
    with _cache_lock:
        if _cache_instance is None:
            try:
                _cache_instance = Redis.from_url(
...
# Keep backward-compatible module-level `cache` reference
cache = None
...
class RengineTask(Task):
    ...
    def __call__(self, *args, **kwargs):
        ...
        if RENGINE_CACHE_ENABLED:
            ...
            _c = _get_cache()
            cached_result = _c.get(record_key) if _c else None
        ...
        try:
            ...
            # Task exceeded its soft time limit (3 hours) — catchable, mark FAILED
            context.status = FAILED_TASK
            context.error = (
                f"Task timed out after 3 hours (soft limit). "
                f"Increase CELERY_TASK_SOFT_TIME_LIMIT if needed. Detail: {exc}"
            )
            ...
            logger.error(
                f"[TIMEOUT] Task '{context.task_name}' scan_id={context.scan_id} "
                f"exceeded soft time limit (3 h) and was terminated gracefully."
            )

        except MemoryError as exc:
            # Worker is running out of memory — fail gracefully
            context.status = FAILED_TASK
            context.error = f"Worker out of memory: {exc}"
            context.traceback = fmt_traceback(exc)
            context.result = context.traceback
            context.output_path = get_traceback_path(
                context.task_name, context.results_dir, context.scan_id, context.subscan_id
            ...
            if context.output_path:
                os.makedirs(os.path.dirname(context.output_path), exist_ok=True)
            logger.error(f"[OOM] Task '{context.task_name}' ran out of memory.")
        ...
        if RENGINE_CACHE_ENABLED and record_key is not None and context.status == SUCCESS_TASK and context.result:
            try:
                _c = _get_cache()
                if _c:
                    _c.set(record_key, json.dumps(context.result))
                    _c.expire(record_key, 600)  # 10 min cache
